...
// RuntimeClient 负责调用独立 trendln runtime。
...
func NewRuntimeClient() *RuntimeClient {
	cfg := config.Get()
	baseURL := strings.TrimSpace(cfg.TrendlnRuntimeBaseURL)
	return NewRuntimeClientWithOptions(baseURL, cfg.TrendlnRuntimeTimeoutMS, baseURL != "")
}
...
func (c *RuntimeClient) doJSON(ctx context.Context, method, path string, input any, output any) (int, error) {
	if !c.enabled() {
		return http.StatusServiceUnavailable, fmt.Errorf("trendln runtime 未启用或未配置")
	}
	...
	if input != nil {
		...
		if err != nil {
			return http.StatusInternalServerError, fmt.Errorf("序列化 trendln 请求失败: %w", err)
		}
	...
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("创建 trendln 请求失败: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")
	client := &http.Client{Timeout: timeout}
	resp, err := client.Do(req)
	if err != nil {
		return http.StatusBadGateway, fmt.Errorf("调用 trendln runtime 失败: %w", err)
	}
	defer resp.Body.Close()
	if output != nil {
		if err := json.NewDecoder(resp.Body).Decode(output); err != nil {
			return resp.StatusCode, fmt.Errorf("解析 trendln 响应失败: %w", err)
		}
