...
import (
	...
	trendln "nofx-trendln"
)

var startedAt = time.Now().UTC().Format(time.RFC3339)

// Start 启动 trendln runtime 的 HTTP 服务。
func Start() error {
	port := runtimePort()
	...
	log.Printf("[trendln-runtime] 已启动 port=%d", port)
	return server.ListenAndServe()
}

func runtimePort() int {
	if raw := os.Getenv("TRENDLN_RUNTIME_PORT"); raw != "" {
...
func handleCompute(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
	var req trendln.ComputeRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{
			"message": "解析 trendln 试算请求失败",
			"error":   err.Error(),
		})
		return
	}
...
func loadCatalog() ([]trendln.FunctionCatalogItem, error) {
	payload, statusCode, err := runBridge("catalog", map[string]any{})
	if err != nil {
		return nil, err
	}
	if statusCode >= http.StatusBadRequest {
		return nil, errTextError("读取 trendln 函数目录失败")
	}
	var result struct {
		Items []trendln.FunctionCatalogItem `json:"items"`
	}
...
func pythonExecutable() (string, error) {
	if raw := strings.TrimSpace(os.Getenv("TRENDLN_PYTHON_PATH")); raw != "" {
		return raw, nil
	}
	...
	if root, err := repoRoot(); err == nil {
		candidate := filepath.Join(root, "app", "backend", "trendln", ".venv", "bin", "python")
...
func bridgeScript() (string, error) {
	...
	return filepath.Join(root, "app", "backend", "trendln", "scripts", "trendln_bridge.py"), nil
}
...
func repoRoot() (string, error) {
	...
	current := wd
	for {
		candidate := filepath.Join(current, "app", "backend", "trendln", "scripts", "trendln_bridge.py")
		if fileExists(candidate) {
			return current, nil
		}
...
func writeJSON(w http.ResponseWriter, status int, payload any) {
	...
	if err := json.NewEncoder(w).Encode(payload); err != nil {
		log.Printf("[trendln-runtime] 写回响应失败: %v", err)
	}
}
