...
// TrendlnTrialHistory 记录每次试算的摘要，便于 admin 追溯。
type TrendlnTrialHistory struct {
...
func (TrendlnTrialHistory) TableName() string {
	return "admin_trendln_trial_histories"
}

type TrendlnTrialHistoryStore struct {
	db *gorm.DB
}

func NewTrendlnTrialHistoryStore(db *gorm.DB) *TrendlnTrialHistoryStore {
	return &TrendlnTrialHistoryStore{db: db}
}

func (s *TrendlnTrialHistoryStore) InitTables() error {
	return s.db.AutoMigrate(&TrendlnTrialHistory{})
}

func (s *TrendlnTrialHistoryStore) Create(item *TrendlnTrialHistory) error {
	if item == nil {
		return fmt.Errorf("trendln trial history is required")
	}
...
func (s *TrendlnTrialHistoryStore) ListRecent(limit int) ([]TrendlnTrialHistory, error) {
	if limit <= 0 {
		limit = 20
	}
	items := make([]TrendlnTrialHistory, 0, limit)
